...
from config import (PROCESSED_DATA_DIR, MODELS_DIR, VIZ_DIR, SKILL_TIERS,
                    RANDOM_STATE, SAMPLE_SIZE, PGN_FILE_PATH)
from src.data_loader import parse_pgn_file, create_games_dataframe, get_dataset_stats
from src.feature_extractor import extract_features_from_dataframe, aggregate_player_features, save_features
from src.classifier import prepare_classification_data, train_classifier, save_model, print_results_summary
from src.clustering import (prepare_clustering_data, find_optimal_k, perform_clustering,
                            analyze_clusters, name_clusters, save_clustering_results, print_clustering_summary)
from src.visualizations import (generate_all_visualizations, create_dashboard_wireframe,
                                plot_skill_distribution, plot_time_heatmap, plot_accuracy_by_tier,
                                plot_confusion_matrix, plot_feature_importance, plot_cluster_embedding)


def load_real_dataset(pgn_path: Path = PGN_FILE_PATH,
                      n_games: int = SAMPLE_SIZE,
                      force_reload: bool = False) -> tuple:
    """
    ...
    Args:
        ...
        force_reload: Force re-parsing even if cached data exists

    Returns:
        Tuple of (games_df, full_games_list) where:
        - games_df: DataFrame with basic game info
        - full_games_list: List of full game dictionaries (with moves/clock data)
    """
    cache_path = PROCESSED_DATA_DIR / f"games_processed_{n_games}.parquet"
    full_cache_path = PROCESSED_DATA_DIR / f"games_full_{n_games}.parquet"

    # Check for cached data
    if cache_path.exists() and full_cache_path.exists() and not force_reload:
        print(f"Loading cached dataset from {cache_path}")
        games_df = pd.read_parquet(cache_path)
        full_games_df = pd.read_parquet(full_cache_path)
    ...
    games_list = []
    for game_data in tqdm(parse_pgn_file(pgn_path, max_games=n_games),
                          total=n_games, desc="Parsing games"):
    ...
    games_df.to_parquet(cache_path)
    full_games_df.to_parquet(full_cache_path)
    print(f"\nCached processed data to {cache_path}")

    return games_df, full_games_df


def generate_synthetic_dataset(n_games: int = 50000,
    ...
    print(f"Unique players: {games_df['white_player'].nunique() + games_df['black_player'].nunique()}")
    print(f"Rating range: {games_df['white_elo'].min()} - {games_df['white_elo'].max()}")

    return games_df


def run_full_pipeline(n_games: int = SAMPLE_SIZE, use_real_data: bool = True,
    ...
    # Find optimal k
    k_results = find_optimal_k(X_cluster.values, k_range=(3, 7))
    optimal_k = k_results['optimal_k']

    # Perform clustering
    clustering_results = perform_clustering(X_cluster, n_clusters=optimal_k, method='kmeans')

    # Analyze and name clusters
    cluster_stats = analyze_clusters(player_features, clustering_results['labels'], feature_cols)
    cluster_names = name_clusters(cluster_stats, player_features, clustering_results['labels'])

    print_clustering_summary(cluster_stats, cluster_names, clustering_results['metrics'])
    ...
    generate_all_visualizations(
        features_df, player_features,
        classification_results, clustering_results,
        cluster_stats, cluster_names
    )
    ...
    summary = {
        'dataset': {
            ...
            'unique_players': games_df['white_player'].nunique() + games_df['black_player'].nunique(),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-games', type=int, default=SAMPLE_SIZE,
